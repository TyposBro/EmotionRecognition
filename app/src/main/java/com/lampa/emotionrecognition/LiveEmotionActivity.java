...
public class LiveEmotionActivity extends AppCompatActivity {
    ...
    private class EmotionAnalyzer implements ImageAnalysis.Analyzer {
        ...
        public void analyze(@NonNull ImageProxy image) {
            ...
            try {
                ...
                // MODIFIED: Pass the ImageProxy to be closed later.
                detectFacesAndEmotions(inputImage, image);
            } catch (Exception e) {
    ...
    // MODIFIED: Method signature changed to accept the ImageProxy.
    private void detectFacesAndEmotions(InputImage inputImage, ImageProxy imageProxy) {
        mFaceDetector.process(inputImage)
                .addOnSuccessListener(faces -> processFaces(faces, inputImage))
                .addOnFailureListener(e -> Log.e(TAG, "Face detection failed", e))
                // MODIFIED: Add an OnCompleteListener to close the image and reset the flag.
    ...
    private void processFaces(List<Face> faces, InputImage inputImage) {
        if (faces.isEmpty()) {
            mMainHandler.post(() -> mEmotionTextView.setText(R.string.no_face_detected));
            return;
        }
        ...
        for (int i = 0; i < faces.size(); i++) {
            ...
            try {
                Bitmap faceBitmap = extractFaceBitmap(inputImage, face.getBoundingBox());
                if (faceBitmap != null) {
                    Map<String, Float> emotions = mEmotionClassifier.classify(faceBitmap, true);
                    String dominantEmotion = getDominantEmotion(emotions);
                    float confidence = emotions.getOrDefault(dominantEmotion, 0f) * 100;
        ...
        final String finalEmotionText = emotionText.toString().trim();
        mMainHandler.post(() -> mEmotionTextView.setText(finalEmotionText));
    }
